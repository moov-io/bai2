...
func (h *AccountTrailer) Parse(data string) (int, error) {
	...
	// AccountControlTotal
	if h.AccountControlTotal, size, err = util.ReadField(util.GetNextData(line, read)); err != nil {
	...
	// NumberRecords
	if h.NumberRecords, size, err = util.ReadFieldAsInt(util.GetNextData(line, read)); err != nil {
