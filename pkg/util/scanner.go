...
func NewScanner(fd io.Reader) Scanner {
	...
	return Scanner{reader: reader, currentLine: currentLine}
}

func (b *Scanner) GetLineIndex() int {
	return b.index
}

func (b *Scanner) GetLine() string {
...
func (b *Scanner) ScanLine(arg ...bool) string {
	...
	for {
		...
		case "/":
			...
			if strings.HasPrefix(line, TransactionDetailCode) || strings.HasPrefix(line, ContinuationCode) {
	...
	fullLine:
		...
		headerCodes := []string{FileHeaderCode, GroupHeaderCode, AccountIdentifierCode, TransactionDetailCode, ContinuationCode, AccountTrailerCode, GroupTrailerCode, FileTrailerCode}
...
func (b *Scanner) atPhysicalLimit(count int) bool {
	return b.physicalRecordLength > 0 && int64(count) >= b.physicalRecordLength
}

func (b *Scanner) skipPhysicalPadding(count *int) {
