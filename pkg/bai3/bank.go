...
// NewBank returns an empty BTR3 bank (record 02) envelope.
func NewBank() *Bank {
	return &Bank{}
}

// Bank is a BTR3 bank envelope (record 02/98). Record 02 is a bank header;
// 98 remains a group trailer in the X9.121 layout.
type Bank struct {
	// Bank Header (02)
	...
	// Group Trailer (98)
	BankControlTotal string `json:"bankControlTotal"`
...
func (r *Bank) copyRecords() {

	r.header = bankHeader{
		Receiver:           r.Receiver,
		BankIdentification: r.BankIdentification,
	...
	r.trailer = bankTrailer{
		BankControlTotal: r.BankControlTotal,
...
func (g *Bank) SumNumberOfAccounts() int64 {
	return int64(len(g.Accounts))
}

// Sums the account control totals in the group. Maps to the BankControlTotal field
func (a *Bank) SumAccountControlTotals() (string, error) {
...
func (r *Bank) String(opts ...int64) string {
...
func (r *Bank) Read(scan *util.Scanner, useCurrentLine bool) error {
	if scan == nil {
	...
	for line := scan.ScanLine(useCurrentLine); line != ""; line = scan.ScanLine(useCurrentLine) {
		...
		case util.GroupHeaderCode:
			...
			r.BankIdentification = newRecord.BankIdentification
